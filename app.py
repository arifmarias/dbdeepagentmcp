...
def deep_insight_function(user_question, agent):
    ...
    try:
        ...
        logger.error(f"Error in deep_insight_function: {str(e)}")
...
def generate_deep_insights(n_clicks, btn_id, chat_history):
    ...
    try:
        # Get user token (same way as call_llm_for_insights)
        headers = request.headers
        user_token = headers.get('X-Forwarded-Access-Token')
        config = Config(
            host=f"https://{os.environ.get('DATABRICKS_HOST')}",
        ...
        # Initialize the model using the SAME serving endpoint
        ...
        # GENIE MCP CONNECTION
        genie_mcp_url = f"{os.environ.get('DATABRICKS_HOST')}/api/2.0/mcp/genie/{GENIE_SPACE_ID}"
        mcp_connection_genie = DatabricksConnection(server_url=genie_mcp_url, workspace_client=client)
        
        
        logger.info("Initializing deep insight agent...")
        
        # Initialize the agent with the Databricks model
        
        # WITH MCP
        agent = create_deep_agent(
            tools=list_databricks_mcp_tools(connections=[mcp_connection_genie,]),
            instructions=RESEARCHER_INSTRUCTIONS,
            subagents=[critique_sub_agent, research_sub_agent],
            model=model,  # Using Databricks serving endpoint
        ...
        # Call the deep insight function with the agent
        ...
        logger.error(f"Error generating deep insights: {str(e)}")
        return html.Div(
            f"Error generating deep insights: {str(e)}", 
            style={"color": "red", "marginTop": "10px"}
        )
